defmodule Servy.Handler do
  ...
  def handle(request) do
    request |> parse |> rewrite_path |> log |> router |> emojify |> track |> format_response
  end
  ...
  def router(%{method: "GET", path: "/bears/" <> id} = conv) do
    %{conv | status: 200, resp_body: "Bear #{id}"}
  end

  def router(%{method: "DELETE", path: "/bears/" <> _id} = conv) do
    %{conv | status: 403, resp_body: "Deleting a bear is forbidden!"}
  end

  def router(%{path: path} = conv) do
